# -------------------------------------------------------------------
#
#  Gmsh Python tutorial for meshing a layered sediment basin
import gmsh
import numpy
#
...
# Units: meter
m = 1  # km in m
r1 = 0.78*m  # rC cannister radius
r2 = 2.2*m  # rB bentonite radius
r3 = 2.25*m  # rZ cement radius
w = 100
h = 100
lc = 1*m    # m
lc1 = 1e-1*m    # m
...
gmsh.model.geo.addPoint(r3, 0, 0, lc3, 3)
gmsh.model.geo.addPoint(w, 0, 0, lc, 4)
gmsh.model.geo.addPoint(w, h, 0, lc, 5)
gmsh.model.geo.addPoint(0, h, 0, lc, 6)
gmsh.model.geo.addPoint(0, r3, 0, lc3, 7)
gmsh.model.geo.addPoint(0, r2, 0, lc2, 8)
gmsh.model.geo.addPoint(0, r1, 0, lc1, 9)
...
gmsh.model.geo.addCircleArc(1, O, 9, 10)
gmsh.model.geo.addCircleArc(2, O, 8, 11)
gmsh.model.geo.addCircleArc(3, O, 7, 12)
...
gmsh.model.geo.addCurveLoop([1, 11, 8, -10], 1)
gmsh.model.geo.addCurveLoop([2, 12, 7, -11], 2)
gmsh.model.geo.addCurveLoop([3, 4, 5, 6, -12], 3)
...
dim = 1
r = gmsh.model.addPhysicalGroup(dim, [4])
gmsh.model.setPhysicalName(dim, r, "right")

b = gmsh.model.addPhysicalGroup(dim, [1, 2, 3])
gmsh.model.setPhysicalName(dim, b, "bottom")

t = gmsh.model.addPhysicalGroup(dim, [5])
gmsh.model.setPhysicalName(dim, t, "top")

l = gmsh.model.addPhysicalGroup(dim, [6, 7, 8])
gmsh.model.setPhysicalName(dim, l, "left")

h = gmsh.model.addPhysicalGroup(dim, [10])
gmsh.model.setPhysicalName(dim, h, "hole")
