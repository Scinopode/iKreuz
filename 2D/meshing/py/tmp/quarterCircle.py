...
# Units: meter
m = 0.001 	# km in m
r1 = 1.05*m	# inner radius
r2 = 100*m	# outer radius
lc = 1e-1*m    # m Netzfeinheit
lc1 = 1e-1*m    # m Netzfeinheit
...
O = gmsh.model.geo.addPoint( 0, 0, 0, lc1)
...
gmsh.model.geo.addLine(O, 1, 1)
gmsh.model.geo.addLine(1, 2, 2)
gmsh.model.geo.addCircleArc(2, O, 3, 3)
gmsh.model.geo.addLine(3, 4, 4)
gmsh.model.geo.addLine(4, O, 5)
gmsh.model.geo.addCircleArc(1, O, 4, 6)
...
gmsh.model.geo.addCurveLoop([1, 6, 5], 1)
gmsh.model.geo.addCurveLoop([2, 3, 4, -6], 2)
...
dim=1
bottom_in = gmsh.model.addPhysicalGroup(dim, [1])
gmsh.model.setPhysicalName(dim, bottom_in, "bottom_in")

bottom_out = gmsh.model.addPhysicalGroup(dim, [2])
gmsh.model.setPhysicalName(dim, bottom_out, "bottom_out")

left_in = gmsh.model.addPhysicalGroup(dim, [5])
gmsh.model.setPhysicalName(dim, left_in, "left_in")

left_out = gmsh.model.addPhysicalGroup(dim, [4])
gmsh.model.setPhysicalName(dim, left_out, "left_out")

inner = gmsh.model.addPhysicalGroup(dim, [6])
gmsh.model.setPhysicalName(dim, inner, "inner")

outer = gmsh.model.addPhysicalGroup(dim, [3])
gmsh.model.setPhysicalName(dim, outer, "outer")
